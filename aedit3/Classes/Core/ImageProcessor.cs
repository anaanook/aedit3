...
namespace aedit.Classes.Core {
    class ImageProcessor {
        /**
         * Reads a Uint32 from a filestream, used specifically by LoadBMP
         */
        ...
        /**
         * Loads only palletized BMP into a texture2D from binary
         */
        public static Texture2D LoadBMP(string file, Texture2D t = null) {
            ...
            //image comes in upside down so there was some annoying math here
            for (int i = 0; i < imgHeight; i++) {
                for (int j = 0; j < imgWidth; j++) {
                    int read = fs.ReadByte();
                    //specifically what im calling with the array, have to go reverse order with i? i guess?
        ...
        /**
         * Takes a given color and makes it transparent in a Texture2D
         */
